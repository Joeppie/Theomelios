...
describe('SearchIndex', () => {
    ...
    describe('search - returns results with details', () => {
        it('returns matches for "God"', () => {
            ...
            const godResult = results.find((r) => r.word.toLowerCase() === 'god');
        ...
        it('returns matches for "beginning"', () => {
            ...
            const beginResult = results.find((r) => r.word === 'beginning');
            if (beginResult) {
    ...
    describe('cross-version search', () => {
        it('finds "casteth" in KJV and related forms in other versions', () => {
            ...
            const castethResult = results.find((r) => r.word === 'casteth');
            if (castethResult && castethResult.matches.length > 0) {
                const kjvMatch = castethResult.matches.find((m) => m.bibleId.includes('King James'));
                expect(kjvMatch).toBeDefined();
            }
        });
        it('finds "cast" via stemming (matching "casteth")', () => {
            const results = library.search('cast');
            const castResult = results.find((r) => r.word.toLowerCase() === 'cast');
            if (castResult) {
    ...
    describe('multi-word search', () => {
        it('prioritizes results containing all query terms', () => {
            ...
            const jesusResult = results.find((r) => r.word.toLowerCase() === 'jesus');
            const weptResult = results.find((r) => r.word.toLowerCase() === 'wept');
            ...
            const termsMatched = jesusResult.matches.map((m) => m.matchingTerms?.length ?? 0);
        ...
        it('sorts same-term-count matches by score', () => {
            const results = library.search('Jesus wept');
            const jesusResult = results.find((r) => r.word.toLowerCase() === 'jesus');
            if (!jesusResult || jesusResult.matches.length < 2)
                return;
            // Find first group of matches with same term count
            for (let i = 0; i < jesusResult.matches.length - 1; i++) {
                const current = jesusResult.matches[i];
                const next = jesusResult.matches[i + 1];
        ...
        it('returns matches from same verse in both word result groups', () => {
            ...
            const jesusResult = results.find((r) => r.word.toLowerCase() === 'jesus');
            const weptResult = results.find((r) => r.word.toLowerCase() === 'wept');
            if (!jesusResult || !weptResult)
                return;
            const jesusKeys = new Set(jesusResult.matches.map((m) => `${m.bibleId}|${m.chapterId}:${m.verseId}`));
            const weptKeys = new Set(weptResult.matches.map((m) => `${m.bibleId}|${m.chapterId}:${m.verseId}`));
