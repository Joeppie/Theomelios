...
import { BibleData, BibleMetadata, ChapterData, TestamentData, BookData, VerseEntry } from '../types/bible'

interface VpcJsonRaw {
  Abbreviation: string
  Language: string
  Publisher?: string
  Copyright?: string
...
function normalizeBookAbbreviation(abbr: string): string {
...
function parseVpcJson(content: string): VpcJsonRaw {
  let processed = content.replace(/(\r\n|\r)/g, '\\n')
  const result = parseJsonValue(processed, 0)
  return result.value as VpcJsonRaw
}
