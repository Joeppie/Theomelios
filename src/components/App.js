...
function App() {
    ...
    const handleFileSelect = async (e) => {
        ...
        try {
            for (let i = 0; i < files.length; i++) {
                const file = files[i];
                if (!file.name.endsWith('.vpc.json'))
                    continue;
    ...
    return (_jsxs("div", { style: { maxWidth: '800px', margin: '0 auto', padding: '2rem', fontFamily: 'system-ui, sans-serif' }, children: [_jsx("h1", { children: "Theomelios" }), _jsx("p", { style: { color: '#666' }, children: "Bible Search Engine" }), _jsxs("div", { style: { marginBottom: '1rem', padding: '1rem', background: '#f5f5f5', borderRadius: '8px' }, children: [_jsx("strong", { children: "Loaded:" }), " ", bibleCount, " bibles, ", docCount.toLocaleString(), " indexed verses"] }), _jsxs("div", { style: { marginBottom: '1rem' }, children: [_jsx("input", { type: "file", multiple: true, accept: ".vpc.json,.json", onChange: handleFileSelect, style: { display: 'block', marginBottom: '0.5rem' } }), _jsx("button", { onClick: handleLoadDemo, disabled: loading, style: { padding: '0.5rem 1rem', background: '#444', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer' }, children: "Load Demo Data" })] }), error && _jsx("p", { style: { color: 'red' }, children: error }), _jsxs("form", { onSubmit: handleSearch, style: { marginBottom: '1rem' }, children: [_jsx("input", { type: "text", value: query, onChange: e => setQuery(e.target.value), placeholder: "Search for a word...", style: { width: '100%', padding: '0.75rem', fontSize: '1rem', border: '1px solid #ddd', borderRadius: '4px', boxSizing: 'border-box' } }), _jsx("button", { type: "submit", disabled: loading, style: { marginTop: '0.5rem', padding: '0.75rem 1.5rem', fontSize: '1rem', background: '#0066cc', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', width: '100%' }, children: loading ? 'Searching...' : 'Search' })] }), results.length > 0 && (_jsxs("div", { children: [_jsxs("h2", { children: ["Results for \"", query, "\" (", results.length, " matches)"] }), results.map((m, j) => (_jsxs("div", { style: { padding: '0.5rem', marginBottom: '0.5rem', background: '#f9f9f9', borderRadius: '4px', borderLeft: `4px solid ${m.matchingTerms.length > 1 ? '#0066cc' : '#999'}` }, children: [_jsxs("div", { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.25rem' }, children: [_jsx("strong", { children: m.bibleName }), " - ", m.bookName, " ", m.chapterId, ":", m.verseId, _jsxs("span", { style: { fontSize: '0.8rem', color: '#666' }, children: ["terms: [", m.matchingTerms.join(', '), "]"] })] }), _jsx("p", { style: { margin: '0', color: '#333' }, children: m.text })] }, j)))] }))] }));
}
export default App;
